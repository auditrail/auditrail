...
- **auditrail-sdk**: add type-checking support with mypy (#3463)
...
- **auditrail-sdk**: include telemetry SDK attributes in tracing (#3359)
...
- **auditrail-sdk**: bump logging instrumentation to support newer otel versions (#3339)
- **auditrail-sdk**: add @staticmethod decorator to set_association_properties (#3341)
...
- **auditrail-sdk**: redefine histogram bucket boundaries (#3129)
...
- **instrumentation**: Adding MCP opentelemetry-instrumentation into auditrail (#2829)
...
- **sdk**: client shouldn't be initialized if destination is not auditrail (#2754)
...
- **auditrail-sdk**: add aiohttp as dependency (#2094)
...
- **sdk**: disable auditrail sync by default (#1835)
...
- **sdk**: otel metrics with auditrail (#883)
...
- **sdk**: warn for reporting score when not using Auditrail (#829)
...
- **auditrail-sdk**: default value for metrics endpoint (#711)
...
- **auditrail-sdk**: custom span processor's on_start is honored (#695)
- **openai**: do not import tiktoken if not used
- **sdk**: exclude api.auditrail.com from requests
...
- **auditrail-sdk**: Log error message when providing wrong API key. (#638)
...
- watsonx support for auditrail (#341)
...
- **auditrail-sdk**: support explicit prompt versioning in prompt management (#221)
...
- auto-create link to auditrail dashboard
...
- **auditrail-sdk**: logging of service name in traces (#99)
...
- **docs**: updated AUDITRAIL_BASE_URL (#81)
...
- **auditrail-sdk**: missing lockfile (#72)
- **auditrail-sdk**: flushing in notebooks (#66)
- py security issue
- **docs**: update exporting.mdx to include nr instrumentation (#12)
- **sdk**: async decorators not awaited
- **sdk**: missing dependency
- warn if Auditrail wasn't initialized properly (#11)
- match new dashboard API
- **auditrail-sdk**: duplicate spans reporting (#10)
- moved api key to /tmp
- /v1/traces is always appended to endpoint
- parse headers correctly
- **auditrail-sdk**:  replace context variables with otel context + refactor (#8)
- auditrail sdk initialization and initial versions release for instrumentations (#7)
