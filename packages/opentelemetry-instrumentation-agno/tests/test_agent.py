...
def test_agent_run_basic(instrument, span_exporter, reader):
    ...
    prompt_content = agent_span.attributes.get(SpanAttributes.AUDITRAIL_ENTITY_INPUT)
    assert prompt_content == "What is 2 + 2?"

    completion_content = agent_span.attributes.get(SpanAttributes.AUDITRAIL_ENTITY_OUTPUT)
...
def test_agent_with_tools(instrument, span_exporter, reader):
    ...
    for tool_span in tool_spans:
        assert tool_span.attributes.get(SpanAttributes.AUDITRAIL_ENTITY_NAME) == "add_numbers"
