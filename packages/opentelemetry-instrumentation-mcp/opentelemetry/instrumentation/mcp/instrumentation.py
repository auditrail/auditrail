...
class McpInstrumentor(BaseInstrumentor):
    ...
    def _fastmcp_client_enter_wrapper(self, tracer):
        ...
        async def traced_method(wrapped, instance, args, kwargs):
            ...
            span.set_attribute(SpanAttributes.AUDITRAIL_SPAN_KIND, "session")
            span.set_attribute(
                SpanAttributes.AUDITRAIL_ENTITY_NAME, "mcp.client.session"
            )
    ...
    async def _handle_tool_call(self, tracer, method, params, args, kwargs, wrapped):
        ...
        with tracer.start_as_current_span(span_name) as span:
            ...
            span.set_attribute(
                SpanAttributes.AUDITRAIL_SPAN_KIND, AuditrailSpanKindValues.TOOL.value
            )
            ...
            if clean_input:
                try:
                    span.set_attribute(
                        SpanAttributes.AUDITRAIL_ENTITY_INPUT, json.dumps(clean_input)
                    )
                except (TypeError, ValueError):
                    span.set_attribute(
                        SpanAttributes.AUDITRAIL_ENTITY_INPUT, str(clean_input)
                    )
    ...
    async def _handle_mcp_method(self, tracer, method, args, kwargs, wrapped):
        ...
        with tracer.start_as_current_span(f"{method}.mcp") as span:
            span.set_attribute(
                SpanAttributes.AUDITRAIL_ENTITY_INPUT, f"{serialize(args[0])}"
            )
            return await self._execute_and_handle_result(
    ...
    async def _execute_and_handle_result(
        ...
        try:
            ...
            if clean_output:
                ...
                span.set_attribute(
                    SpanAttributes.AUDITRAIL_ENTITY_OUTPUT, serialize(result)
                )
            # Handle errors
