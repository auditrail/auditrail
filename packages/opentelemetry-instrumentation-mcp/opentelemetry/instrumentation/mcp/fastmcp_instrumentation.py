...
from opentelemetry.semconv_ai import SpanAttributes, AuditrailSpanKindValues
...
class FastMCPInstrumentor:
    ...
    def _fastmcp_tool_wrapper(self):
        ...
        async def traced_method(wrapped, instance, args, kwargs):
            ...
            with self._tracer.start_as_current_span("mcp.server") as mcp_span:
                mcp_span.set_attribute(SpanAttributes.AUDITRAIL_SPAN_KIND, "server")
                mcp_span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_NAME, "mcp.server")
                if self._server_name:
                    mcp_span.set_attribute(SpanAttributes.AUDITRAIL_WORKFLOW_NAME, self._server_name)

                # Create nested tool span
                span_name = f"{entity_name}.tool"
                with self._tracer.start_as_current_span(span_name) as tool_span:
                    tool_span.set_attribute(SpanAttributes.AUDITRAIL_SPAN_KIND, AuditrailSpanKindValues.TOOL.value)
                    tool_span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_NAME, entity_name)
                    if self._server_name:
                        tool_span.set_attribute(SpanAttributes.AUDITRAIL_WORKFLOW_NAME, self._server_name)
                    ...
                    if self._should_send_prompts():
                        try:
                            ...
                            tool_span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_INPUT, truncated_input)
                    ...
                    try:
                        # Add output in auditrail format to tool span
                        if self._should_send_prompts() and result:
                            try:
                                ...
                                tool_span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_OUTPUT, truncated_output)
    ...
    def _should_send_prompts(self):
        """Check if content tracing is enabled (matches auditrail SDK)"""
        return (
            os.getenv("AUDITRAIL_TRACE_CONTENT") or "true"
        ).lower() == "true"

    def _get_json_encoder(self):
        """Get JSON encoder class (simplified - auditrail SDK uses custom JSONEncoder)"""
        return None  # Use default JSON encoder

    def _truncate_json_if_needed(self, json_str: str) -> str:
        """Truncate JSON if it exceeds OTEL limits (matches auditrail SDK)"""
