async def test_fastmcp_server_mcp_parent_span(span_exporter, tracer_provider) -> None:
    ...
    assert server_span.attributes.get('auditrail.span.kind') == 'server', \
        "Server span should have server span kind"
    assert server_span.attributes.get('auditrail.entity.name') == 'mcp.server', \
    ...
    assert tool_span.attributes.get('auditrail.entity.name') == 'test_tool', \
