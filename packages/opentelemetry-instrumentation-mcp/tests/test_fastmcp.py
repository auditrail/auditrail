async def test_fastmcp_instrumentor(span_exporter, tracer_provider) -> None:
    ...
    for i, span in enumerate(tool_call_spans):
        ...
        assert span.attributes.get('auditrail.span.kind') == 'tool', (
            f"Span {i} should have tool span kind"
        )
        assert span.attributes.get('auditrail.entity.name') == 'add_numbers', (
            f"Span {i} should have correct entity name"
        )

        # Verify actual input content
        input_attr = span.attributes.get('auditrail.entity.input', '')
        ...
        output_attr = span.attributes.get('auditrail.entity.output', '')
    ...
    resource_input = resource_read_span.attributes.get('auditrail.entity.input', '')
    ...
    resource_output = resource_read_span.attributes.get('auditrail.entity.output', '')
    ...
    # Verify AUDITRAIL_WORKFLOW_NAME is set correctly on server spans
    ...
    for server_span in mcp_server_spans:
        workflow_name = server_span.attributes.get('auditrail.workflow.name')
        assert workflow_name == 'test-server.mcp', (
            f"Expected workflow name 'test-server.mcp', got '{workflow_name}'"
        )

    # Verify AUDITRAIL_WORKFLOW_NAME is also set on tool spans
    server_tool_spans = [span for span in spans if span.name == 'add_numbers.tool'
                         and span.attributes.get('auditrail.span.kind') == 'tool'
                         and 'auditrail.workflow.name' in span.attributes]
    ...
    for tool_span in server_tool_spans:
        workflow_name = tool_span.attributes.get('auditrail.workflow.name')
        assert workflow_name == 'test-server.mcp', (
