...
async def test_fastmcp_comprehensive_attributes(span_exporter, tracer_provider) -> None:
    ...
    # Test 1: Verify span naming follows auditrail pattern
    assert tool_span.name == "process_data.tool"

    # Test 2: Verify auditrail attributes
    assert tool_span.attributes.get("auditrail.span.kind") == "tool"
    assert tool_span.attributes.get("auditrail.entity.name") == "process_data"
    assert tool_span.attributes.get("auditrail.workflow.name") == "attribute-test-server.mcp"
    ...
    input_attr = tool_span.attributes.get("auditrail.entity.input")
    ...
    output_attr = tool_span.attributes.get("auditrail.entity.output")
...
async def test_fastmcp_error_handling(span_exporter, tracer_provider) -> None:
    ...
    if error_spans:
        ...
        assert (
            ...
            "error" in error_span.attributes.get("auditrail.entity.output", "").lower()
        )

        # Verify span still has correct auditrail attributes
        assert error_span.attributes.get("auditrail.span.kind") == "tool"
        assert error_span.attributes.get("auditrail.entity.name") == "failing_tool"

        # Verify workflow name is set correctly even on error spans
        assert error_span.attributes.get("auditrail.workflow.name") == "error-test-server.mcp"
