...
class AuditrailCallbackHandler(BaseCallbackHandler):
    def __init__(
    ...
    def _create_span(
        ...
        _set_span_attribute(span, SpanAttributes.AUDITRAIL_WORKFLOW_NAME, workflow_name)
        _set_span_attribute(span, SpanAttributes.AUDITRAIL_ENTITY_PATH, entity_path)
    ...
    def _create_task_span(
        ...
        _set_span_attribute(span, SpanAttributes.AUDITRAIL_SPAN_KIND, kind.value)
        _set_span_attribute(span, SpanAttributes.AUDITRAIL_ENTITY_NAME, entity_name)
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        kind = (
            ...
            else AuditrailSpanKindValues.TASK
        )

        if kind == AuditrailSpanKindValues.WORKFLOW:
            workflow_name = name
        else:
            workflow_name = self.get_workflow_name(parent_run_id)
            entity_path = self.get_entity_path(parent_run_id)
