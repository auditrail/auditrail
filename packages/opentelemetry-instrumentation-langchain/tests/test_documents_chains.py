...
def test_sequential_chain(instrument_legacy, span_exporter, log_exporter):
    ...
    data = json.loads(stuff_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT])
    assert data["inputs"].keys() == {"input_documents"}
    assert data["kwargs"]["name"] == "stuff_chain"
    data = json.loads(stuff_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT])
