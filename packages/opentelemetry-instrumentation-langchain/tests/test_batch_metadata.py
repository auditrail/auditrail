...
def test_batch_metadata_in_span_attributes(instrument_legacy, span_exporter):
    ...
    for span in llm_spans:
        ...
        user_id_key = f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.user_id"
        session_id_key = f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.session_id"
        priority_key = f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.priority"
...
async def test_async_batch_metadata_in_span_attributes(instrument_legacy, span_exporter):
    ...
    for span in llm_spans:
        ...
        user_id_key = f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.user_id"
        session_id_key = f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.session_id"
        environment_key = f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.environment"
