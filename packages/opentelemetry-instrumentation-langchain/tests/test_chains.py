...
def test_sequential_chain(instrument_legacy, span_exporter, log_exporter):
    ...
    assert workflow_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND] == "workflow"
    assert (
        workflow_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME]
        == "SequentialChain"
    )
    assert all(
        span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND] == "task"
        for span in task_spans
    )
    assert all(
        span.attributes[SpanAttributes.AUDITRAIL_WORKFLOW_NAME] == "SequentialChain"
        for span in spans
    )
    assert all(
        span.attributes[SpanAttributes.AUDITRAIL_ENTITY_PATH]
    ...
    data = json.loads(synopsis_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT])
    assert data["inputs"] == {
        "title": "Tragedy at sunset on the beach",
        "era": "Victorian England",
    }
    assert data["kwargs"]["name"] == "synopsis"
    data = json.loads(synopsis_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT])
    assert data["outputs"].keys() == {
        "synopsis",
    }

    data = json.loads(review_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT])
    assert data["inputs"].keys() == {"title", "era", "synopsis"}
    assert data["kwargs"]["name"] == "LLMChain"
    data = json.loads(review_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT])
    assert data["outputs"].keys() == {
        "review",
    }
    ...
    data = json.loads(overall_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT])
    assert data["inputs"] == {
        "title": "Tragedy at sunset on the beach",
        "era": "Victorian England",
    }
    assert data["kwargs"]["name"] == "SequentialChain"
    data = json.loads(overall_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT])
