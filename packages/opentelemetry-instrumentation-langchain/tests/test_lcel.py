...
def test_simple_lcel(instrument_legacy, span_exporter, log_exporter):
    ...
    assert json.loads(
        workflow_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT]
    ) == {
    ...
    assert json.loads(
        workflow_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT]
    ) == {
        "outputs": {
    ...
    assert json.loads(
        prompt_task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT]
    ) == {
        "inputs": {"input": "tell me a short joke"},
    ...
    assert json.loads(
        prompt_task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT]
    ) == {
