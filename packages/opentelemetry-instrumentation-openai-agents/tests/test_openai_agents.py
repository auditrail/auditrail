...
def test_agent_spans(exporter, test_agent):
    ...
    assert (
        agent_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND]
...
def test_agent_with_function_tool_spans(exporter, function_tool_agent):
    ...
    assert (
        tool_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND]
...
def test_agent_with_handoff_spans(exporter, handoff_agent):
    ...
    assert (
        agent_a_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND]
        == AuditrailSpanKindValues.AGENT.value
    )
...
async def test_recipe_workflow_agent_handoffs_with_function_tools(
    ...
    if tool_spans:
        ...
        if search_tool_spans:
            ...
            assert (
                search_tool_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND]
                == AuditrailSpanKindValues.TOOL.value
            )
