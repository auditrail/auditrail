...
from opentelemetry.semconv_ai import AuditrailSpanKindValues

from auditrail.sdk.decorators.base import (
    entity_class,
    entity_method,
)
...
def workflow(
    ...
    version: Optional[int] = None,
    method_name: Optional[str] = None,
    tlp_span_kind: Optional[AuditrailSpanKindValues] = AuditrailSpanKindValues.WORKFLOW,
) -> Callable[[F], F]:
    if method_name is None:
        return entity_method(name=name, version=version, tlp_span_kind=tlp_span_kind)
    else:
...
def agent(
    ...
    return workflow(
        ...
        version=version,
        method_name=method_name,
        tlp_span_kind=AuditrailSpanKindValues.AGENT,
    )
...
def tool(
    ...
    return task(
        ...
        version=version,
        method_name=method_name,
        tlp_span_kind=AuditrailSpanKindValues.TOOL,
    )
