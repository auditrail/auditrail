...
from auditrail.sdk.tracing import get_tracer, set_workflow_name, set_agent_name
from auditrail.sdk.tracing.tracing import (
    TracerWrapper,
    set_entity_path,
    get_chained_entity_path,
)
from auditrail.sdk.utils import camel_to_snake
from auditrail.sdk.utils.json_encoder import JSONEncoder
...
def aentity_method(
    ...
    tlp_span_kind: Optional[AuditrailSpanKindValues] = AuditrailSpanKindValues.TASK,
):
    warnings.warn(
...
def _setup_span(entity_name, tlp_span_kind, version):
    ...
    if tlp_span_kind in [
        AuditrailSpanKindValues.WORKFLOW,
        AuditrailSpanKindValues.AGENT,
    ]:
        set_workflow_name(entity_name)

    if tlp_span_kind == AuditrailSpanKindValues.AGENT:
    ...
    with get_tracer() as tracer:
        ...
        if tlp_span_kind in [
            AuditrailSpanKindValues.TASK,
            AuditrailSpanKindValues.TOOL,
        ]:
            entity_path = get_chained_entity_path(entity_name)
            set_entity_path(entity_path)

        span.set_attribute(SpanAttributes.AUDITRAIL_SPAN_KIND, tlp_span_kind.value)
        span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_NAME, entity_name)
        if version:
            span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_VERSION, version)
...
def _handle_span_input(span, args, kwargs, cls=None):
    ...
    try:
        if _should_send_prompts():
            ...
            span.set_attribute(
                SpanAttributes.AUDITRAIL_ENTITY_INPUT,
                truncated_json,
            )
    except TypeError:
        pass
...
def _handle_span_output(span, res, cls=None):
    ...
    try:
        if _should_send_prompts():
            ...
            span.set_attribute(
                SpanAttributes.AUDITRAIL_ENTITY_OUTPUT,
                truncated_json,
            )
    except TypeError:
        pass
