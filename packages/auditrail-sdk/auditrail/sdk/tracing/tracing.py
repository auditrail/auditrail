...
from auditrail.sdk.tracing.content_allow_list import ContentAllowList
from auditrail.sdk.utils import is_notebook
from auditrail.sdk.utils.package_check import is_package_installed
...
class TracerWrapper(object):
    ...
    def verify_initialized(cls) -> bool:
        ...
        if (os.getenv("AUDITRAIL_SUPPRESS_WARNINGS") or "false").lower() == "true":
            return False

        print(
            Fore.RED
            + "Warning: Auditrail not initialized, make sure you call Auditrail.init()"
        )
        print(Fore.RESET)
        return False
...
def _set_association_properties_attributes(span, properties: dict) -> None:
    for key, value in properties.items():
        span.set_attribute(
            f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.{key}", value
        )
...
def default_span_processor_on_start(span: Span, parent_context: Context | None = None):
    ...
    if workflow_name is not None:
        span.set_attribute(SpanAttributes.AUDITRAIL_WORKFLOW_NAME, str(workflow_name))
    ...
    if entity_path is not None:
        span.set_attribute(SpanAttributes.AUDITRAIL_ENTITY_PATH, str(entity_path))
    ...
    if is_llm_span(span):
        ...
        if managed_prompt is not None:
            span.set_attribute(
                SpanAttributes.AUDITRAIL_PROMPT_MANAGED, str(managed_prompt)
            )

        prompt_key = get_value("prompt_key")
        if prompt_key is not None:
            span.set_attribute(SpanAttributes.AUDITRAIL_PROMPT_KEY, str(prompt_key))

        prompt_version = get_value("prompt_version")
        if prompt_version is not None:
            span.set_attribute(
                SpanAttributes.AUDITRAIL_PROMPT_VERSION, str(prompt_version)
            )

        prompt_version_name = get_value("prompt_version_name")
        if prompt_version_name is not None:
            span.set_attribute(
                SpanAttributes.AUDITRAIL_PROMPT_VERSION_NAME, str(prompt_version_name)
            )

        prompt_version_hash = get_value("prompt_version_hash")
        if prompt_version_hash is not None:
            span.set_attribute(
                SpanAttributes.AUDITRAIL_PROMPT_VERSION_HASH, str(prompt_version_hash)
            )

        prompt_template = get_value("prompt_template")
        if prompt_template is not None:
            span.set_attribute(
                SpanAttributes.AUDITRAIL_PROMPT_TEMPLATE, str(prompt_template)
            )
        ...
        if prompt_template_variables is not None and isinstance(
            ...
            for key, value in prompt_template_variables.items():
                span.set_attribute(
                    f"{SpanAttributes.AUDITRAIL_PROMPT_TEMPLATE_VARIABLES}.{key}",
                    value,
                )
...
def get_default_span_processor(
    ...
    setattr(processor, "_auditrail_processor", True)
...
def metrics_common_attributes():
    ...
    workflow_name = get_value("workflow_name")
    if workflow_name is not None:
        common_attributes[SpanAttributes.AUDITRAIL_WORKFLOW_NAME] = workflow_name

    entity_name = get_value("entity_name")
    if entity_name is not None:
        common_attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME] = entity_name

    association_properties = get_value("association_properties")
    if association_properties is not None:
        for key, value in association_properties.items():
            common_attributes[
                f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.{key}"
            ] = value
