...
from auditrail.sdk.prompts.registry import PromptRegistry
from auditrail.sdk.prompts.client import PromptRegistryClient
from auditrail.sdk.tracing.content_allow_list import ContentAllowList
from auditrail.sdk.version import __version__

MAX_RETRIES: int = int(os.getenv("AUDITRAIL_PROMPT_MANAGER_MAX_RETRIES") or 3)
POLLING_INTERVAL: int = int(os.getenv("AUDITRAIL_PROMPT_MANAGER_POLLING_INTERVAL") or 5)
...
class Fetcher:
    ...
    def post(self, api: str, body: Dict[str, str]) -> None:
        post_url(f"{self._base_url}/v1/auditrail/{api}", self._api_key, body)
...
def fetch_url(url: str, api_key: str) -> Any:
    response = requests.get(
        ...
        headers={
            ...
            "X-Auditrail-SDK-Version": __version__,
        },
    )

    if response.status_code != 200:
        if response.status_code == 401 or response.status_code == 403:
            logging.error("Authorization error: Invalid Auditrail API key.")
...
def refresh_data(
    ...
    response = fetch_url(f"{base_url}/v1/auditrail/prompts", api_key)
    prompt_registry.load(response)

    response = fetch_url(f"{base_url}/v1/auditrail/pii/tracing-allow-list", api_key)
