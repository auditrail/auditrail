...
def test_association_properties(exporter):
    ...
    Auditrail.set_association_properties({"user_id": 1, "user_name": "John Doe"})
    test_workflow()
    ...
    assert (
        some_workflow_span.attributes[
            f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.user_id"
        ]
        == 1
    )
    assert (
        some_workflow_span.attributes[
            f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.user_name"
        ]
        == "John Doe"
    )
    assert (
...
def test_association_properties_within_workflow(exporter):
    ...
    def test_workflow():
        Auditrail.set_association_properties({"session_id": 15})
        return
    ...
    assert (
        some_workflow_span.attributes[
            f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.session_id"
        ]
        == 15
    )
...
def test_langchain_and_external_association_properties(exporter):
    ...
    def test_workflow_external():
        Auditrail.set_association_properties({"workspace_id": "789"})
    ...
    assert (
        workflow_span.attributes[
            f"{SpanAttributes.AUDITRAIL_ASSOCIATION_PROPERTIES}.workspace_id"
        ]
        == "789"
    )
    assert (
