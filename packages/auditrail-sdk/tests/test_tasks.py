...
def test_task_io_serialization_with_langchain(exporter):
    ...
    assert json.loads(task_span.attributes.get(SpanAttributes.AUDITRAIL_ENTITY_OUTPUT))["kwargs"]["content"] == "Yes"
...
def test_json_truncation_with_otel_limit(exporter, monkeypatch):
    ...
    input_json = task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT]
    ...
    output_json = task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT]
...
def test_json_no_truncation_without_otel_limit(exporter, monkeypatch):
    ...
    input_data = json.loads(task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT])
    assert input_data["args"][0] == long_input

    # Check that output was not truncated
    output_data = json.loads(task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT])
