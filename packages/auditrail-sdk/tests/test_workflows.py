...
from auditrail.sdk.decorators import workflow, task
...
def test_simple_workflow(exporter, openai_client):
    ...
    def create_something(what: str, subject: str):
        Auditrail.set_prompt("Tell me a {what} about {subject}", {"what": what, "subject": subject}, 5)
    ...
    assert open_ai_span.attributes.get("auditrail.prompt.template") == "Tell me a {what} about {subject}"
    assert open_ai_span.attributes.get("auditrail.prompt.template_variables.what") == "joke"
    assert open_ai_span.attributes.get("auditrail.prompt.template_variables.subject") == "OpenTelemetry"
    assert open_ai_span.attributes.get("auditrail.prompt.version") == "5"
    ...
    assert json.loads(task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_INPUT]) == {
        "args": ["joke"],
        "kwargs": {"subject": "OpenTelemetry"},
    }

    assert json.loads(task_span.attributes.get(SpanAttributes.AUDITRAIL_ENTITY_OUTPUT)) == joke
    assert task_span.parent.span_id == workflow_span.context.span_id
    assert workflow_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME] == "pirate_joke_generator"
    assert workflow_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_VERSION] == 1
    assert task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_VERSION] == 2
...
def test_unrelated_entities(exporter):
    ...
    assert workflow_1_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME] == "workflow_1"
    assert workflow_1_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND] == "workflow"

    assert task_1_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME] == "task_1"
    assert task_1_span.attributes[SpanAttributes.AUDITRAIL_SPAN_KIND] == "task"
...
def test_dataclass_serialization_workflow(exporter):
    ...
    assert json.loads(task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_OUTPUT]) == {
        "field1": "value1",
        "field2": 123,
    }
    assert task_span.parent.span_id == workflow_span.context.span_id
    assert workflow_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME] == "dataclass_workflow"
    assert task_span.attributes[SpanAttributes.AUDITRAIL_ENTITY_NAME] == "dataclass_task"
