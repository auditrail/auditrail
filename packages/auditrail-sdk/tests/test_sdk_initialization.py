...
def test_multiple_span_processors(exporters_with_multiple_span_processors):
    ...
    # Verify that the default processor (Auditrail) added its attributes
    default_span = default_spans[0]
    # The default processor should have Auditrail-specific attributes
...
def test_get_default_span_processor():
    ...
    processor = Auditrail.get_default_span_processor(disable_batch=True)
    assert isinstance(processor, SimpleSpanProcessor)
    assert hasattr(processor, "_auditrail_processor")
    assert getattr(processor, "_auditrail_processor") is True

    # Test with batch enabled
    processor = Auditrail.get_default_span_processor(disable_batch=False)
